    ...
    public boolean place(MovingObjectPosition block, boolean extra) {
        ...
        if (sneak.isToggled()) {
            if (sneak$bridged >= sneakEveryBlocks.getInput()) {
                ...
                Raven.getExecutor().schedule(() -> mc.gameSettings.keyBindSneak.pressed = (Keyboard.isKeyDown(mc.gameSettings.keyBindSneak.getKeyCode())), (long) sneakTime.getInput(), TimeUnit.MILLISECONDS);
            }
        }
